 ...
 * Guaranteed to fit 100% on a single page with zero awkward spills or mid-page cuts.
 */
    ...
    try {
      ...
      if (resumeJson.skills && Object.keys(resumeJson.skills).length > 0) {
        ...
        Object.entries(resumeJson.skills).forEach(([category, skillsList]) => {
          ...
          doc.font('Helvetica')
             .fontSize(8.5)
             .fillColor(textColor)
             .text(listStr, { width: contentWidth, lineGap: 1 });
          doc.moveDown(0.12);
        });
      }
      ...
      if (resumeJson.experience && resumeJson.experience.length > 0) {
        ...
        resumeJson.experience.forEach(job => {
          ...
          // Line 1: Role and Duration on same line
          doc.font('Helvetica-Bold')
             .fontSize(9)
             .fillColor(textColor)
          ...
          if (job.highlights && Array.isArray(job.highlights)) {
            job.highlights.forEach(bullet => {
              doc.font('Helvetica')
                 .fontSize(8.5)
                 .fillColor(textColor)
                 .text(`•  ${bullet}`, leftMargin, doc.y, { indent: 10, width: contentWidth, lineGap: 1 });
              doc.moveDown(0.05);
            });
          }
      ...
      if (resumeJson.achievements && resumeJson.achievements.length > 0) {
        ...
        resumeJson.achievements.forEach(ach => {
          ...
          doc.font('Helvetica')
             .fontSize(8.5)
             .fillColor(textColor)
             .text(`•  ${ach}`, leftMargin, doc.y, { indent: 10, width: contentWidth, lineGap: 1 });
          doc.moveDown(0.05);
        });
      }
